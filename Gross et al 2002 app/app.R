...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Gross et al. 2022 Atlantic sturgeon matrix population model"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      h3('Model and vital rates from Gross et al. 2002 and literature'),
      h5('Content below is static'),
      tags$figure(
        class = "centerFigure",
        tags$img(
          src = "Gross et al 2002 model.png",
          width = 600
        ),
        tags$figcaption("Life cycle a la Gross et al. 2002")
      ),
...
server <- function(input, output) {
  ...
  # solve for lambda
  lambda_solve <- function(pop = popmat(), lamb = input$lambda){
